 ...
 * @brief Manages interactive tutorials by highlighting UI elements and guiding the user.
 *
 * This class implements a step-by-step overlay system that visually highlights
 * widgets and shows textual instructions to guide the user through the interface.
 * It supports multiple tutorials (e.g., for LL(1) and SLR(1) modes), with custom steps
 * and signals for tutorial completion.
 */
...
public:
     ...
     * @param rootWindow The main application window used for relative positioning.
     */
    ...
    /**
     * @brief Ends the LL(1) tutorial sequence and emits its corresponding signal.
     */
    void finishLL1();

    /**
     * @brief Ends the SLR(1) tutorial sequence and emits its corresponding signal.
     */
    void finishSLR1();
...
private:
    ...
    QFrame *m_highlight = nullptr;     ///< Highlight rectangle around the target widget.
